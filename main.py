...
def fetch_inputs(inputs,  output_file=None,url='url missing'):
    for input_ in inputs:
        input_name = input_.get('name')
        input_value = input_.get('value')
        input_count=len(inputs)
        print(yellow(f'working on {url}'))
        ...
        if output_file:
            output_file.write(f"{url}: {input_name if input_name else 'empty_name'}: {input_value if input_value else 'empty_value'}\n")
...
def fetch_froms(url,output_file=None,time_out=5,delay_=0):
    ...
    if forms:
        ...
        for form in forms:
            ...
            if output_file:
                output_file.write(f"{url}:{action}:{method}")
    ...
    try:
        ...
        if input_file:
                ...
                try:
                    ...
                    if random_agent:
                        ...
                        if hidden:
                            ...
                            if inputs:
                                fetch_inputs(inputs,output_file,i)
                            forms=soap.find_all("form")
                            if forms:
            ...
            try:
                ...
                inputs=url_soap.find_all("input")
                if inputs:
                    fetch_inputs(inputs,output_file,url)
                if forms:
                    fetch_froms(url,output_file,time_out,delay_)
            except KeyboardInterrupt: 
                print(red("\nSee You Soon Hacker !!"))
